...
sources:
  - name: external_source
    meta:
      external_location: "./seeds/{name}.xls"
    tables:
      - name: seeds_source
      ...
      - name: seeds_ost
        ...
        meta:
          external_location: "./seeds/{identifier}.xls"
"""
...
class TestExternalSources:
    ...
    def test_external_sources(self, project):
        ...
        seeds_source.to_excel("seeds/seeds_source.xls", index=False)
        seeds_other_source_table = pd.DataFrame(
            {"id": [1, 4, 7], "c": [2, 5, 8], "d": [3, 6, 9]}
        )
        seeds_other_source_table.to_excel(
            "seeds/seeds_other_source_table.xls", index=False
        )
